...
public class OpenGLDefaultMaterialRenderer implements MeshRenderer {
    ...
    public void render(MeshMaterialBatch batch) {
        ...
        GL.vaoContext(batch.getVaoID(), () -> {
            ...
            GL41.glEnableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.VX_NORMAL.location);

            if (batch.getMaterial().hasDiffuseTexture()) {
                GL41.glEnableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.VX_UV_COORDS.location);
                GL41.glEnableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.INS_UV_SCALE.location);
                GL41.glEnableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.INS_UV_OFFSET.location);
            ...
            GL41.glDisableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.VX_NORMAL.location);

            if (batch.getMaterial().hasDiffuseTexture()) {
                GL41.glDisableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.VX_UV_COORDS.location);
                GL41.glDisableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.INS_UV_SCALE.location);
                GL41.glDisableVertexAttribArray(OpenGLDefaultGPUMeshDataStore.LayoutQualifier.INS_UV_OFFSET.location);
